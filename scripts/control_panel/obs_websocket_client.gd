## Based on the obs-websocket-gd addon:
...
class ObsMessage:
	...
	func get_data(skip_empty: bool = false) -> Variant:
		var dict := {"d": {}}
		for i in get_property_list():
			...
			if prop_name == "op":
				...
				dict["d"][prop_name] = prop

		return dict

	func get_as_dict(skip_empty: bool = false) -> Dictionary:
		return get_data(skip_empty)

	func get_as_json(skip_empty: bool = false) -> String:
		return JSON.stringify(get_data(skip_empty), "\t")
...
func _handle_data_received() -> int:
	var message := _get_message()
...
func send_command_batch(commands: Array) -> void:
